...
canvas.addEventListener("touchstart", touchStart, false);
    ...
    symmetryElement = document.getElementById('symmetryElement');
    symmetryElement.addEventListener('click',symmetryFunc,false);
...
function mouseMove(e){
	...
	if(symmetry){
		...
		if(this.drawX>canvas.width/2){
			this.x2=(canvas.width/2)-(this.drawX-canvas.width/2);
			//toolList(e,"move",this.drawX,this.drawY);
			toolList2(e,"move",this.x2,this.drawY);
			//toolList(e,"move",this.drawX,this.drawY);
...
function touchEnd(e){
e.preventDefault();
	this.drawX = e.targetTouches[0].pageX-canvas.offsetLeft;
	this.drawY = e.targetTouches[0].pageY-canvas.offsetTop;
...
function toolList2(e,mouseEvent,xx,yy){
...
//up
	...
	switch(tool){
		...
		case "fillRectangle":
		fillRectangleObj2.mouseUp(e,xx,yy);
		break;
...
//down
	...
	switch(tool){
		...
		case "fillRectangle":
		fillRectangleObj2.mouseDown(e,xx,yy);
		break;
...
//move
	...
	switch(tool){
		...
		case "fillRectangle":
		fillRectangleObj2.mouseMove(e,xx,yy);
		break;
...
document.getElementById("mess").innerHTML=document.getElementById("mess").innerHTML+" "+lineWidth;
...
saveImage();
	...
	for(var i=0;i<imgData.length;i+=4){
		/*imgData[i]=255-Math.sin(imgData[i])*200;
		imgData[i+1]=255-Math.sin(imgData[i+1])*200;
		imgData[i+2]=255-Math.sin(imgData[i+2])*200;
		//imgData[i+3]=255-Math.sin(imgData[i+3])*200*/
if( (imgData[i]!=imgData[i+4*img.width] && imgData[i+1]!=imgData[i+4*img.width+1] && imgData[i+1+1]!=imgData[i+4*img.width+1+1]  && i<imgData.length-4*img.width)  ||  (imgData[i]!=imgData[i+4] && imgData[i+1]!=imgData[i+4+1] && imgData[i+1+1]!=imgData[i+4+1+1]  && i<img.data.length-4)  ){
imgData[i]=0;
imgData[i+1]=0;
imgData[i+2]=0;
...
// (imgData[i]!=imgData[i-4] && imgData[i+1]!=imgData[i-4+1] && imgData[i+1+1]!=imgData[i-4+1+1]  && i>4)  ||  
...
var pixel  = (fbx + fby * imageData.width) * 4


for(var i=0;i<this.fba.length;i++){
if( imgData[pixel]!=imgData[pixel+4] && imgData[pixel+1]!=imgData[pixel+4+1] && imgData[pixel+2]!=imgData[pixel+4+2] && pixel<4*imd.data.width*img.data.height-7){

add(pixel+4);

}

if( imgData[pixel]!=imgData[pixel-4] && imgData[pixel+1]!=imgData[pixel-4+1] && imgData[pixel+2]!=imgData[pixel-4+2] && pixel>=4 ){

add(pixel-4);

}

imgData[pixel+4*imgData.width*4]=125;
imgData[pixel+4*imgData.width*4+1]=125;
imgData[pixel+4*imgData.width*4+2]=125;
...
if(this.fxWidth<this.dummyLineWidth){
this.fxWidth+=0.5*2;
lineWidth=this.fxWidth;
paint();
bum();
}

    ctx.beginPath();
    ctx.arc(this.drawX,this.drawY,lineWidth/100,0,2*Math.PI,false);
	ctx.fill();
